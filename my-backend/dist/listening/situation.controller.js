"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SituationController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const situation_service_1 = require("./situation.service");
let SituationController = class SituationController {
    situationService;
    constructor(situationService) {
        this.situationService = situationService;
    }
    findAll() {
        return this.situationService.findAll();
    }
    findOneWithDetails(id) {
        return this.situationService.findOneWithDetails(id);
    }
    updateProgress(id, updateDto, req) {
        const userId = req.user?.userId || '000000000000000000000000';
        return this.situationService.updateProgress(userId, id, updateDto.progress_seconds);
    }
};
exports.SituationController = SituationController;
__decorate([
    (0, common_1.Get)(),
    (0, swagger_1.ApiOperation)({ summary: 'Lấy danh sách các tình huống luyện nghe' }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], SituationController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)(':id/details'),
    (0, swagger_1.ApiOperation)({ summary: 'Lấy chi tiết nội dung tình huống (Audio & Kịch bản)' }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], SituationController.prototype, "findOneWithDetails", null);
__decorate([
    (0, common_1.Post)(':id/progress'),
    (0, swagger_1.ApiOperation)({ summary: 'Cập nhật tiến độ học tập' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __param(2, (0, common_1.Request)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Object, Object]),
    __metadata("design:returntype", void 0)
], SituationController.prototype, "updateProgress", null);
exports.SituationController = SituationController = __decorate([
    (0, swagger_1.ApiTags)('Situations'),
    (0, common_1.Controller)('situations'),
    __metadata("design:paramtypes", [situation_service_1.SituationService])
], SituationController);
//# sourceMappingURL=situation.controller.js.map